"""
Pairwise VAR and Granger Causality Network Analysis - NORMALIZED
Z-score ALL rank percentages to level the playing field

FIX: Enlisted percentages (E4=20%, E5=15%) dominate officer percentages (O4=3%, O8=0.5%)
SOLUTION: Z-score normalize ALL rank percentages before analysis
"""
...
print("=" * 100)
print("PAIRWISE VAR AND GRANGER CAUSALITY - WITH Z-SCORE NORMALIZATION")
print("All rank percentages normalized to level the playing field")
print("=" * 100)

# =============================================================================
# LOAD AND NORMALIZE DATA
# =============================================================================
print("\n[1/6] Loading and normalizing dataset...")

df = pd.read_excel('data/analysis/complete_normalized_dataset_v10.6_FULL.xlsx')

# Already normalized variables (keep as-is)
already_normalized = [
    'Policy_Count_Log',
    'Total_PAS_Z',
    'Total_Civilians_Z',
    'FOIA_Simple_Days_Z',
    'FOIA_Complex_Days_Z',
    'GDP_Growth',  # Already a growth rate
    'Major_Conflict'  # Binary
]

# Rank percentages to normalize
rank_pcts = [
...
# Political variables (binary, keep as-is)
political = ['HOR_Republican', 'Senate_Republican', 'President_Republican']

print("\n  Z-score normalizing rank percentages:")
print("  " + "-" * 96)
...
for rank in rank_pcts:
    if rank in df.columns:
        ...
        std_val = df[rank].std()
        df[f'{rank}_Z'] = (df[rank] - mean_val) / std_val
        print(f"    {rank:20s} -> {rank}_Z (mean={mean_val:6.2f}%, std={std_val:5.2f}%)")

# Collect all variables for analysis (using normalized versions)
analysis_vars = (
    already_normalized +
    [f'{rank}_Z' for rank in rank_pcts if rank in df.columns] +
    political
)
...
granger_df.to_excel('data/analysis/pairwise_granger_all_NORMALIZED.xlsx', index=False)

granger_sig = granger_df[granger_df['Significant_5pct']].copy()
granger_sig.to_excel('data/analysis/pairwise_granger_significant_NORMALIZED.xlsx', index=False)
...
edges_df.to_excel('data/analysis/pairwise_network_edges_NORMALIZED.xlsx', index=False)
...
centrality_df.to_excel('data/analysis/pairwise_network_centrality_NORMALIZED.xlsx', index=False)
...
print(f"\n  TOP {top_n} VARIABLES SELECTED (Z-SCORE NORMALIZED):")
print("  " + "=" * 96)
...
selection_df.to_excel('data/analysis/top_variables_for_var_NORMALIZED.xlsx', index=False)
...
labels = {node: node.replace('_Z', '') if node in top_vars else '' for node in G.nodes()}
nx.draw_networkx_labels(G, pos, labels, font_size=8, font_weight='bold', ax=ax)

ax.set_title(f'Causal Network (Z-Score Normalized) - Top {top_n} Variables Highlighted',
...
ax.set_yticks(y_pos)
ax.set_yticklabels([v.replace('_Z', '') for v in top_20['Variable']], fontsize=9)
ax.set_xlabel('Composite Centrality Score', fontsize=12, fontweight='bold')
ax.set_title('Top 20 Variables by Network Centrality (Normalized)',
            fontsize=16, fontweight='bold', pad=20)
...
plt.tight_layout()
plt.savefig('data/analysis/pairwise_network_analysis_NORMALIZED.png', dpi=300, bbox_inches='tight')
...
# =============================================================================
print("\n" + "=" * 100)
print("ANALYSIS COMPLETE - SUMMARY (Z-SCORE NORMALIZED)")
print("=" * 100)
...
print("=" * 100)
print("  1. pairwise_granger_all_NORMALIZED.xlsx")
print("  2. pairwise_granger_significant_NORMALIZED.xlsx")
print("  3. pairwise_network_edges_NORMALIZED.xlsx")
print("  4. pairwise_network_centrality_NORMALIZED.xlsx")
print("  5. top_variables_for_var_NORMALIZED.xlsx")
print("  6. pairwise_network_analysis_NORMALIZED.png")

print("\n" + "=" * 100)
print("KEY FIX APPLIED:")
print("=" * 100)
print("  ✓ All rank percentages z-score normalized")
print("  ✓ Levels playing field between enlisted (high %) and officers (low %)")
print("  ✓ Prevents magnitude bias in Granger causality tests")
print("=" * 100)
